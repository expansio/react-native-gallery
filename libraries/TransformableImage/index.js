...
export default class TransformableImage extends PureComponent {
  ...
  getImageSize(image) {
    ...
    const { source, dimensions, width, height } = image;
        ...
        Image.getSize(
          ...
          (imgWidth, imgHeight) => {
            if (imgWidth && imgHeight) {
              if (
                this.state.imageDimensions &&
                this.state.imageDimensions.width === imgWidth &&
                this.state.imageDimensions.height === imgHeight
              ) {
                // no need to update state
              } else {
                this._mounted &&
                  this.setState({ imageDimensions: { width: imgWidth, height: imgHeight } });
              }
            }
          },
          () => {
    ...
    return (
      <View>
        ...
        {!imageLoaded && (
          ...
          >
            <ActivityIndicator color="#3266BC" />
          </View>
        )}
      </View>
    );
